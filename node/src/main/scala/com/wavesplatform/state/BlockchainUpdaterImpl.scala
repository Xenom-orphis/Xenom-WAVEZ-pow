          ...
          (ngState match {
            ...
            case Some(ng) =>
              if (ng.base.header.reference == block.header.reference) {
                // PoW blocks (rewardVote == -1) can replace liquid blocks regardless of timestamp
                val isPowBlock = block.header.rewardVote == -1L
                val canReplace = isPowBlock || (block.header.timestamp < ng.base.header.timestamp)
            ...
            _ map {
              case (
                ...
                // PoW Mining Rewards: Logged for PoW blocks (rewardVote == -1)

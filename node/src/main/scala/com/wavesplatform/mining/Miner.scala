...
class MinerImpl(
  ...
  def scheduleMining(tempBlockchain: Option[Blockchain]): Unit = {
    // PoS mining disabled - this is a pure PoW blockchain
    // All block generation happens via external PoW miner and /mining/submit API
    log.trace("PoS mining disabled - using PoW-only mode")
    debugStateRef = MinerDebugInfo.Disabled
  }

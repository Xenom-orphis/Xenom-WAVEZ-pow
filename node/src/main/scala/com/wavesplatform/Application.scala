...
import com.wavesplatform.api.http.{BlockHeaderRoutes, BlockStorage}
...
class Application(val actorSystem: ActorSystem, val settings: WavesSettings, configRoot: ConfigObject, time: NTP) extends ScorexLogging {
  ...
  // BlockStorage implementation for BlockHeaderRoutes
  // Maps Waves SignedBlockHeader to consensus.BlockHeader for PoW mining
  private val blockStorage = new BlockStorage {
    override def getBlockHeaderByHeight(height: Long): Option[_root_.consensus.BlockHeader] = {
      if (height == 0) {
        // Return the genesis PoW block
        Some(_root_.consensus.Genesis.Block)
      } else if (height > 0 && height <= Int.MaxValue) {
        // Map Waves blockchain header to PoW header format
